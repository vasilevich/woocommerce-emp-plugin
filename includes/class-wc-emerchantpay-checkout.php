    ...
    public function payment_fields()
    {
        ...
        if ($this->is_select_only_one_enabled()) {
            try {
                ...
                foreach ($transaction_types as $key => $data) {
                    echo '<option data-currencies="' . esc_attr(json_encode($data['currencies'])) . '" value="' . esc_attr($key) . '">' . esc_html($data['label']) . '</option>';
                }
                echo '</select>';

                echo '<script>
(() => {
    const currency = jQuery(".wcml-dropdown.product.wcml_currency_switcher .wcml-cs-item-toggle").text().trim();
    const select = jQuery("#' . self::SETTING_KEY_TRANSACTION_TYPES_TO_LABEL . '");

    select.find("option").each(function () {
        try {
            const allowedCurrencies = JSON.parse(jQuery(this).attr("data-currencies") || "[]");
            // If "ALL" is in the allowed currencies, skip the check
            if (!allowedCurrencies.includes("ALL") && !allowedCurrencies.includes(currency)) {
                jQuery(this).remove();
            }
        } catch (error) {
    ...
    // Refresh the select box if needed
